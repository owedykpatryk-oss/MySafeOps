/**
 * Vercel serverless proxy for CH postcode → lat/lng (Nominatim, Switzerland only).
 * GET /api/ch-postcode?code=8001
 */
...
const USER_AGENT = "MySafeOps/1.0 (CH construction safety; support@mysafeops.com)";
...
function normalizeChPostcode(raw) {
...
export default async function handler(req, res) {
  ...
  if (rejectIfRateLimited(req, res, "ch-postcode", { max: 30, windowMs: 60_000 })) {
    return;
  }

  const code = normalizeChPostcode(String(req.query?.code || req.query?.postcode || ""));
  if (!code) {
    return sendJson(res, 400, { error: "Invalid Swiss postcode (4 digits)" });
  }
