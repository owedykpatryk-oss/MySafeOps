/**
 * Vercel serverless proxy for AT postcode → lat/lng (Nominatim, Austria only).
 * GET /api/at-postcode?code=1010
 */
...
const USER_AGENT = "MySafeOps/1.0 (AT construction safety; support@mysafeops.com)";
...
function normalizeAtPostcode(raw) {
...
export default async function handler(req, res) {
  ...
  if (rejectIfRateLimited(req, res, "at-postcode", { max: 30, windowMs: 60_000 })) {
    return;
  }

  const code = normalizeAtPostcode(String(req.query?.code || req.query?.postcode || ""));
  if (!code) {
    return sendJson(res, 400, { error: "Invalid Austrian postcode (4 digits)" });
  }
