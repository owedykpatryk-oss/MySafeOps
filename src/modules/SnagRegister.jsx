...
const STATUSES = {
  open: { label: "Open", bg: "#FCEBEB", color: "#791F1F" },
...
function SnagForm({ snag, workers, projects, onSave, onClose }) {
  ...
  const autoRef = () => {
    if (!form.ref) {
      const r = `SN-${String(load("snag_counter", 1)).padStart(3, "0")}`;
      save("snag_counter", (load("snag_counter", 1) || 1) + 1);
      set("ref", r);
    }
  };
  useEffect(autoRef, []);
    ...
    >
      <div style={{ ...ss.card, width: "100%", maxWidth: 560 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <PhotoCapture
            photos={form.photos || []}
            onChange={(fn) => setForm((f) => ({ ...f, photos: typeof fn === "function" ? fn(f.photos || []) : fn }))}
          />
        </div>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
            {snag.ref && (
              <span style={{ fontSize: 11, color: "var(--color-text-secondary)", fontWeight: 500 }}>{snag.ref}</span>
            )}
            <Badge type="status" value={snag.status} />
            <Badge type="priority" value={snag.priority} />
            {snag.category && (
              <span
                style={{ fontSize: 11, padding: "2px 8px", borderRadius: 20, background: "#E6F1FB", color: "#0C447C" }}
              >
                {snag.category}
              </span>
            )}
            {overdue && (
              <span
                style={{ fontSize: 11, padding: "2px 8px", borderRadius: 20, background: "#FCEBEB", color: "#791F1F" }}
              >
                Overdue
              </span>
            )}
          ...
          {snag.description && (
            <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 6, lineHeight: 1.5 }}>
              {snag.description.slice(0, 120)}
              {snag.description.length > 120 ? "…" : ""}
            </div>
          )}

          <div
            style={{ display: "flex", gap: 16, fontSize: 11, color: "var(--color-text-secondary)", flexWrap: "wrap" }}
          >
            {snag.location && <span>📍 {snag.location}</span>}
            {snag.assignedTo && <span>👤 {workerMap[snag.assignedTo] || snag.assignedTo}</span>}
            {snag.dueDate && (
              <span style={{ color: overdue ? "#A32D2D" : "inherit" }}>Due: {fmtDate(snag.dueDate)}</span>
            )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
          <select
            value={snag.status}
            onChange={(e) => onStatusChange(snag.id, e.target.value)}
...
export default function SnagRegister() {
  ...
  const saveSnag = (snag) => {
    ...
    setSnags((prev) =>
      prev.find((s) => s.id === snag.id) ? prev.map((s) => (s.id === snag.id ? snag : s)) : [snag, ...prev]
    );
    setModal(null);
  };
  const deleteSnag = (id) => {
  ...
  const exportSelectedCsv = () => {
    ...
    a.download = `snags_selected_${today()}.csv`;
    a.click();
  };
  const bulkMarkClosed = () => {
    ...
    const ids = selected;
    setSnags((prev) =>
      prev.map((s) => (ids.has(s.id) ? { ...s, status: "closed", closedAt: new Date().toISOString() } : s))
    );
    setSelected(new Set());
  };
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    a.download = `snag_register_${today()}.csv`;
    a.click();
  };
          ...
          snags.length > 0 ? (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <input
                ...
                placeholder="Search…"
                style={{ ...ss.inp, width: "auto", minWidth: 140, flex: 1 }}
              />
              <select
                value={filterStatus}
