...
export default function WaterHygieneLog() {
  ...
  const { d1Hydrating: d1ItemsH, d1OutboxPending: d1ItemsO } = useD1OrgArraySync({
    storageKey: "water_hygiene_log",
    namespace: "water_hygiene_log",
    value: items,
    setValue: setItems,
    load,
    save,
  ...
  return (
    <div style={{ fontFamily: "DM Sans,system-ui,sans-serif", padding: "1.25rem 0", fontSize: 14 }}>
      <D1ModuleSyncBanner d1Hydrating={d1Hydrating} d1OutboxPending={d1OutboxPending} scopeLabel="water hygiene log" />
      {modal?.type === "form" && <Form item={modal.data} projects={projects} onSave={(f) => persist(f, !modal.data)} onClose={() => setModal(null)} />}
            <PageHero exportModuleId="water-hygiene"
        badgeText="WH"
        title="Water hygiene"
                      ...
                      onClick={() => {
                        if (
                          softDeleteToRecycleBin({
                            moduleId: "water-hygiene",
                            moduleLabel: "Water hygiene",
                            itemType: "water_check",
                            itemLabel: r.outletId || r.checkDate || r.id,
                            sourceKey: "water_hygiene_log",
                            payload: r,
                          })
                        ) {
