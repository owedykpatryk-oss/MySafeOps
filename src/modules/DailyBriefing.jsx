...
function SigCanvas({ onCapture, compact }) {
  ...
  const clear = () => {
    ref.current.getContext("2d").clearRect(0, 0, 500, 160);
    setHas(false);
  };
...
function BriefingForm({ onSave, onClose, workers, projects, initial = null }) {
  ...
  const toggleAttendee = (id) =>
    set(
      "attendees",
      form.attendees.map((a) => (a.id === id ? { ...a, present: !a.present, sig: null } : a))
    );
  ...
  const setAttSig = (id, dataUrl) =>
    set(
      "attendees",
      form.attendees.map((a) => (a.id === id ? { ...a, sig: dataUrl, sigTime: new Date().toISOString() } : a))
    );
    ...
    >
      <div style={{ ...ss.card, width: "100%", maxWidth: 620 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginTop: 2 }}>
              Pre-work safety briefing record
            </div>
          </div>
          <button onClick={onClose} style={{ ...ss.btn, padding: "4px 8px" }}>
            ×
          </button>
        </div>
          ...
          <div style={{ gridColumn: "1/-1" }}>
            ...
            {workers.length > 0 ? (
              <>
                <select
                  value={conductPick}
                  onChange={onConductSelect}
                  style={{ ...ss.inp, marginBottom: conductPick === "__custom__" ? 8 : 0 }}
                >
        ...
        <div style={{ marginBottom: 16 }}>
          <textarea
            value={form.scopeToday}
            onChange={(e) => set("scopeToday", e.target.value)}
            rows={2}
            placeholder="What is the planned scope of work for today?"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 8 }}>
          {BRIEF_TOPICS.map((t) => (
            <label
              key={t}
              style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", fontSize: 13 }}
            >
              <input
                type="checkbox"
        ...
        <div style={{ marginBottom: 16 }}>
            ...
            >
              <div
                style={{
                  ...
                  alignItems: "center",
                  gap: 10,
                  marginBottom: a.present && !a.sig && signingId === a.id ? 10 : 0,
                }}
              >
                <input
                  type="checkbox"
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
            ...
            <button
              ...
              style={{ ...ss.btn, fontSize: 12, opacity: guestName.trim() ? 1 : 0.45 }}
            >
              + Add guest
            </button>
          </div>
    ...
    <div className="app-daily-briefing-card" style={{ ...ss.card, marginBottom: 8, contentVisibility: "auto", containIntrinsicSize: "0 96px" }}>
      ...
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>
              {fmtDate(brief.date)} · {brief.time}
            </span>
          </div>
          <div
            style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "flex", gap: 12, flexWrap: "wrap" }}
          >
            {brief.conductedBy && <span>By: {brief.conductedBy}</span>}
            <span>
              {presentCount} attended · {signedCount} signed
            </span>
            <span>{brief.topics?.length || 0} topics</span>
            {brief.weatherConditions && (
              <span>
                {brief.weatherConditions}
                {brief.temperature ? ` · ${brief.temperature}°C` : ""}
              </span>
            )}
          </div>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 18, color: "var(--color-text-secondary)", paddingTop: 2 }}>
            {expanded ? "▲" : "▼"}
          </span>
        </div>
      </div>
                ...
                .map((a) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "4px 10px",
                      borderRadius: 20,
                      background: a.sig ? "#EAF3DE" : "var(--color-background-secondary,#f7f7f5)",
                      border: "0.5px solid var(--color-border-tertiary,#e5e5e5)",
                    }}
                  >
                    <span style={{ fontSize: 12, color: a.sig ? "#27500A" : "var(--color-text-primary)" }}>
                      {a.name}
                    </span>
                    {a.sig && (
                      <svg width={10} height={10} viewBox="0 0 10 10" fill="none">
                        <path d="M2 5l2.5 2.5 4-4" stroke="#27500A" strokeWidth={1.5} strokeLinecap="round" />
                      </svg>
                    )}
                  </div>
                ))}
...
export default function DailyBriefing() {
  ...
  const filtered = briefings.filter((b) => {
    ...
    if (
      ...
      !b.conductedBy?.toLowerCase().includes(search.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  return (
    ...
    >
      <D1ModuleSyncBanner
        ...
        scopeLabel="briefings and team lists"
      />
      {showForm && (
        <BriefingForm
          workers={workers}
          ...
          briefings.length > 0 ? (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <input
                ...
                placeholder="Search location or conductor…"
                style={{ ...ss.inp, flex: 1, width: "auto", minWidth: 140 }}
              />
              <input
                type="date"
                value={filterDate}
              ...
              {projects.length > 0 ? (
                <select
                  ...
                  style={{ ...ss.inp, width: "auto", minWidth: 140 }}
                >
