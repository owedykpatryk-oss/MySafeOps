...
import {
  ...
  buildCoverHeroCollageHtml,
  buildRevisionVerifyBlockHtml,
...
import {
  ...
  SURVEY_REVISION_DIFF_CSS,
  buildRecordsRevisionDiff,
} from "./surveyFieldUpgrades.js";
...
function coverPage(report, org, primary, accent, extras) {
  ...
  if (isUtilityMappingPrintTheme()) {
    ...
    const shareUrl = report.ref
      ? buildOrgShareUrlWithRef(
          org,
          report.ref,
          UTILITY_MAPPING_BRAND.website || "https://u-map.co.uk/",
          dc.revision || ""
        )
      : "";
    const collage = buildCoverHeroCollageHtml(report, extras);
    return `${renderUtilityMappingHeroCover({
      ...
      qrLabel: [report.ref, dc.revision ? `Rev ${dc.revision}` : ""].filter(Boolean).join(" · "),
      meta: [
...
export function buildSurveyReportHtml(report, extras = {}) {
  ...
  let recordsDiff = r.recordsRevisionDiff;
  if (!recordsDiff?.hasChanges && r.parentReportId && extras.parentReport) {
    recordsDiff = buildRecordsRevisionDiff(
      extras.parentReport.revisionBaselineSnapshot || extras.parentReport,
      r
    );
  }
  if (recordsDiff?.hasChanges) {
    pushSection("Records revision compare", "rev-records-diff", buildRevisionDiffHtml(recordsDiff));
  }
  ...
  try {
    // Survey-first strip (method / QL / records) — dig-risk omitted; digging is a later client activity.
    const insight = buildCoverInsightStripHtml(r, null);
    if (insight) {
  ...
  const revForQr = String(r.documentControl?.revision || "").trim();
  const verifyUrl = r.ref
    ? buildOrgShareUrlWithRef(
        org,
        r.ref,
        umTheme ? UTILITY_MAPPING_BRAND.website || "https://u-map.co.uk/" : undefined,
        revForQr
      )
    : "";
  const revQrSrc = verifyUrl ? buildUtilityMappingQrSrc(verifyUrl, 120) : "";
  const revVerify = buildRevisionVerifyBlockHtml(r, revQrSrc, verifyUrl);
  if (revVerify) {
    pushSection("Revision verification", "rev-verify", revVerify);
  }
  ...
  const watermarkText = umTheme
    ? r.status === "final"
      ? `CONTROLLED · ${r.ref || "UM"}${dc.revision ? ` · REV ${dc.revision}` : ""}`
      : `DRAFT · ${r.ref || "UM"}${dc.revision ? ` · REV ${dc.revision}` : ""}`
    : String(org.pdfWatermarkText || "").trim() ||
      (r.status === "final"
        ? `FINAL${dc.revision ? ` · REV ${dc.revision}` : ""}`
        : `DRAFT${dc.revision ? ` · REV ${dc.revision}` : ""}`);
  const umThemeCss = umTheme
    ? `${utilityMappingCoverSystemCss()}${utilityMappingSurveyCoverCss(primary, accent)}${utilityMappingPremiumPagesCss(primary, accent)}${SURVEY_EVIDENCE_PRINT_CSS}${SURVEY_PLAN_UPGRADE_CSS}${SURVEY_REVISION_DIFF_CSS}`
    : `${SURVEY_EVIDENCE_PRINT_CSS}${SURVEY_PLAN_UPGRADE_CSS}${SURVEY_REVISION_DIFF_CSS}`;
