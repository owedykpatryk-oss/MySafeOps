  ...
  return (
    <div style={{ fontFamily: "DM Sans,system-ui,sans-serif", padding: "1.25rem 0", fontSize: 14 }}>
      ...
      <RegisterModuleShell moduleId="vehicles" smartContext={{ items }} stats={buildRegisterModuleStats("vehicles", items)}>
        ...
        {items.length === 0 ? (
          <div style={{ ...ss.card, textAlign: "center", color: "var(--color-text-secondary)" }}>
            No vehicles on the register. Add vans, cars or HGVs with MOT and insurance dates.
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {listPg.hasMore(items) ? (
              <div style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>
                Showing {Math.min(listPg.cap, items.length)} of {items.length} vehicles
              </div>
            ) : null}
