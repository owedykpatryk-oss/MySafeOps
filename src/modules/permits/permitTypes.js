...
export const PERMIT_TYPES = {
  ...
  excavation: {
    ...
    checklist: [
      ...
      "PAS 128 survey level (QL) reviewed — matches dig method on drawing",
      "Type B1+ detection or Type A verification completed where required",
    ...
    extraFields: [
      ...
      { key:"pas128QualityLevel", label:"PAS 128 quality level (QL-D to QL-A)", type:"text" },
      { key:"pas128SurveyType", label:"PAS 128 survey type (D / C / B1 / B2 / B3 / A)", type:"text" },
      { key:"surveyDrawingRef", label:"Utility survey / drawing reference", type:"text" },
      { key:"horizontalAccuracyMm", label:"Stated horizontal accuracy (mm)", type:"number" },
      { key:"mechanicalDigAllowed", label:"Mechanical dig permitted", type:"text" },
      { key:"trialPitDone", label:"Trial pit / vacuum verification", type:"text" },
  ...
  ground_disturbance: {
    ...
    checklist: [
      ...
      "PAS 128 QL and survey type (B1+ or A) recorded on permit",
  ...
  line_clearance: {
    label: "Line clearance / product isolation",
    color: "#633806", bg: "#FAEEDA",
    icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z",
    description: "Food factory — mechanical work adjacent to open product lines; production isolated before entry",
    checklist: [
      "Production manager sign-off — line stopped and cleared",
      "Product isolation verified — no open product in work zone",
      "Allergen changeover completed if crossing allergen zones",
      "Tools and materials foreign-body checked (no glass, loose fixings)",
      "G&HP register checked for items brought into zone",
      "LOTO / isolation locks applied and verified",
      "Hygiene barrier and boot-change procedure followed",
      "Line restart briefing scheduled with production team",
    ],
    extraFields: [
      { key: "productionLine", label: "Production line / zone", type: "text" },
      { key: "isolationRef", label: "Isolation / LOTO reference", type: "text" },
      { key: "productionSignOff", label: "Production manager sign-off name", type: "text" },
    ],
  },
  rail_corridor_access: {
    label: "Rail corridor access permit",
    color: "#185FA5", bg: "#E6F1FB",
    icon: "M4 15h16M4 10h16M6 6h12v12H6z",
    description: "Survey or works within rail corridor — possession, PTS and safe system of work",
    checklist: [
      "Possession / access window confirmed in writing with infrastructure manager",
      "All personnel hold valid PTS (or client equivalent) and medical fitness",
      "Lookout / COSS arrangements confirmed for non-possession works",
      "Isolation of OLE / third rail confirmed where applicable",
      "Emergency egress route and muster point briefed",
      "Site-specific rail induction completed for all team members",
      "Equipment and materials secured — no foul of running line",
      "Handback inspection completed before line reopen",
    ],
    extraFields: [
      { key: "possessionRef", label: "Possession / access reference", type: "text" },
      { key: "ptsNumbers", label: "PTS numbers (lead + team)", type: "text" },
      { key: "cossName", label: "COSS / lookout name", type: "text" },
    ],
  },
  ole_isolation: {
    label: "OLE / conductor rail isolation permit",
    color: "#B45309", bg: "#FDF1DF",
    icon: "M13 2L3 14h9l-1 8 10-12h-9l1-8zM2 21h20",
    description: "Overhead line or conductor rail isolated and earthed before work inside the exclusion zone",
    checklist: [
      "Isolation requested and granted by the authorised person (OLEC / NP)",
      "Isolation limits and earthing positions confirmed in writing",
      "Portable earths / short-circuiting straps applied and recorded",
      "Proving dead completed on the section to be worked",
      "Exclusion zone (600 mm live OLE / third rail) marked and briefed",
      "No conductive equipment raised near live sections outside the isolation",
      "Adjacent lines and DC return bonding arrangements confirmed",
      "Earths removed and isolation surrendered only by the permit holder",
    ],
    extraFields: [
      { key: "isolationRef", label: "Isolation / OLEC reference", type: "text" },
      { key: "authorisedPerson", label: "Authorised person (isolation)", type: "text" },
      { key: "earthPositions", label: "Earth positions applied", type: "text" },
    ],
  },
  on_track_plant: {
    label: "On-track plant / RRV permit",
    color: "#0F766E", bg: "#E2F3F0",
    icon: "M6 4h12v9H6zM4 17h16M8 20h8M9 13v4M15 13v4",
    description: "Road-rail vehicles, tampers and on-track machines — access point, controller and slew control",
    checklist: [
      "Machine Controller / Crane Controller appointed and briefed",
      "Access point agreed and inspected before on-tracking",
      "Plant certificates, LOLER examination and RRV competence verified",
      "Slew and travel limits agreed; adjacent line open/closed status confirmed",
      "Exclusion zone maintained around slewing plant — no one in the danger zone",
      "Lifting operations covered by a lift plan and appointed person",
      "Fuel, oil and coolant spill kit available at the access point",
      "Off-tracking and site clearance recorded before handback",
    ],
    extraFields: [
      { key: "plantType", label: "Machine type / fleet number", type: "text" },
      { key: "machineController", label: "Machine / Crane Controller name", type: "text" },
      { key: "accessPoint", label: "Access point (mileage / ref)", type: "text" },
    ],
  },
  marine_hydrographic: {
    label: "Marine / hydrographic works permit",
    color: "#0F6E8C", bg: "#E0F4F8",
    icon: "M2 18c2-4 6-4 8 0s6 4 8 0M2 12c2-4 6-4 8 0s6 4 8 0",
    description: "Bathymetric, hydrographic or vessel-based survey operations",
    checklist: [
      "Marine risk assessment and MOB drill completed",
      "Vessel/RHIB competency and insurance verified",
      "Weather, tide and current limits agreed — stop-work criteria set",
      "PFD/life jacket mandatory for all deck operations",
      "Harbour master / port authority notification where required",
      "Emergency shore contact and rescue plan in place",
      "Survey equipment secured against overboard loss",
      "Fuel and chemical handling per COSHH on vessel",
    ],
    extraFields: [
      { key: "vesselName", label: "Vessel / craft name", type: "text" },
      { key: "coxswain", label: "Coxswain / master name", type: "text" },
      { key: "tideWindow", label: "Approved tide / weather window", type: "text" },
    ],
  },
  aerial_survey_coordination: {
    label: "Aerial survey coordination permit",
    color: "#633806", bg: "#FAEEDA",
    icon: "M12 2L2 7l10 5 10-5-10-5zm0 9l2.5-1.25L22 12l-7.5 3.75L12 17l-2.5-1.25L2 12l7.5-3.75L12 11z",
    description: "Drone, helicopter or fixed-wing survey — airspace, NOTAM and ground exclusion",
    checklist: [
      "Flight plan and risk assessment approved by survey lead",
      "CAA operator authorisation / A2 CofC or GVC verified for UAV",
      "NOTAM or airspace coordination completed for manned aircraft",
      "Landowner and client notifications for low-level overflight",
      "Ground exclusion zone established under flight path",
      "Weather limits (wind, visibility, precipitation) agreed",
      "Emergency landing area identified for UAV operations",
      "Post-flight equipment check and secure storage completed",
    ],
    extraFields: [
      { key: "aircraftType", label: "Aircraft / UAV type", type: "text" },
      { key: "operatorId", label: "CAA operator ID / pilot licence", type: "text" },
      { key: "notamRef", label: "NOTAM / airspace clearance ref", type: "text" },
    ],
  },
  general: {
