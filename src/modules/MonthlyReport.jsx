...
import { escapeHtml, safeCssColor, safeImageSrc, openPrintWindow, escapeAttr, writePrintWindowDocument } from "../utils/htmlEscape.js";
...
function printMonthlyReport(monthLabel) {
  ...
  if (!win) return;
  const brand = safeCssColor(org.primaryColor, "#0d9488");
  const logoSrc = safeImageSrc(org.logo);
  await writePrintWindowDocument(win, `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Monthly H&S summary — ${escapeHtml(monthLabel)}</title>
  <style>
    body { font-family: Arial, sans-serif; font-size: 12px; color: #000; margin: 0; padding: 20px; }
    h1 { background: ${brand}; color: #fff; padding: 8px 12px; font-size: 15px; }
    table { width: 100%; border-collapse: collapse; margin-top: 12px; }
    th, td { border: 1px solid #ccc; padding: 6px 8px; text-align: left; }
    th { background: #f4f4f4; }
    @media print { h1 { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }
  </style></head><body>
  <div style="display:flex;align-items:center;gap:16px;border-bottom:2px solid ${brand};padding-bottom:10px;margin-bottom:14px">
    ${logoSrc ? `<img src="${escapeAttr(logoSrc)}" alt="" style="height:44px;max-width:120px;object-fit:contain"/>` : ""}
    <div>
      <div style="font-weight:bold;font-size:14px">${escapeHtml(org.name || "MySafeOps")}</div>
      ${org.pdfHeader ? `<div style="font-size:11px;color:#666">${escapeHtml(org.pdfHeader)}</div>` : ""}
    </div>
  </div>
  <h1>Monthly health & safety summary — ${escapeHtml(monthLabel)}</h1>
  ...
  <p style="font-size:10px;color:#999;margin-top:16px">${escapeHtml(org.pdfFooter || "Generated by MySafeOps")}</p>
  </body></html>`);
  win.print();
