...
| Klient | `src/lib/d1SyncClient.js` — wywołania do Workera; przy błędzie HTTP zwraca m.in. `request_id` z nagłówka `X-Request-Id`; błąd sieci (`fetch`) → `fetch_failed` bez rzucania. Hook `src/hooks/useD1OrgArraySync.js` (+ `useD1WorkersProjectsSync`) — wiele modułów/rejestrów (lista i wyjątki: [BACKEND_CONTINUATION_PLAN.md](./BACKEND_CONTINUATION_PLAN.md)): hydratacja z D1 (do 3 prób GET), zapis z debounce + **jedna ponowiona próba PUT** przy `502/503/504/429`, `localStorage` jako cache; konflikt wersji → refetch z serwera. **Kolejka offline:** `src/lib/d1SyncOutbox.js` (IndexedDB `mysafeops_d1_outbox`) — po nieudanym PUT (po retry, z wyjątkiem 409) ostatnia wartość jest zapisywana lokalnie i wysyłana ponownie po hydratacji, zdarzeniu `online` lub co ~45 s, dopóki się nie uda. **Ręczny retry:** `src/lib/d1OutboxRetryEvent.js` (`requestD1OutboxManualRetry`) — każda zamontowana instancja hooka próbuje flush swojego klucza; przycisk w `D1ModuleSyncBanner.jsx`. Hook zwraca `d1Hydrating`, `d1OutboxPending`, `d1Syncing` (suma); UI: `src/components/D1ModuleSyncBanner.jsx` — osobny komunikat przy kolejce („upload queued”) vs hydratacja („Syncing … with cloud”), `role="status"` / `aria-live`. |
...
- **Prawda wyłącznie na serwerze**: aplikacja nadal inicjuje z `localStorage` i **natychmiast** zapisuje lokalnie; D1 jest autorytatywne po udanej hydratacji; nieudany GET nadal pozwala na pracę offline z cache (jak wcześniej). **Kolejka IndexedDB** ogranicza utratę zapisów przy chwilowej niedostępności API; pełne „tylko API, zero zaufania do klienta” lub blokada edycji do `d1Ready` to osobna decyzja produktowa.
...
- **Cyber — dowód w CI:** `.github/workflows/ci.yml` uruchamia m.in. `npm audit --audit-level=high` oraz Playwright m.in. `tests/e2e/security.spec.js` (`/.well-known/security.txt` + `/security`).
