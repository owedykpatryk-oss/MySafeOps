/** Outbound webhook URL validation for Edge Functions (SSRF guard). */
...
export function validateOutboundWebhookUrl(
  ...
  if (BLOCKED_HOSTS.has(host)) return { ok: false, error: "URL host is not allowed" };
  ...
  if (protocol === "https:") {
    // ok
  } else if (protocol === "http:" && opts.allowHttpLocalhost && isLocalhost) {
    // ok
  } else {
...
export function detectIncomingWebhookKind(url: string): "slack" | "teams" | "generic" {
