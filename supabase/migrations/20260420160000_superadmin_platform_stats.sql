-- Platform-wide metrics for the product owner only (mysafeops@gmail.com).
-- SECURITY DEFINER bypasses RLS; access is gated by JWT email inside the function.
--
-- Note: Avoid EXECUTE ... INTO <var> here — some SQL runners misparse INTO and treat the
-- target as a relation (42P01). RHS of := uses PL variable substitution as parameters.
...
begin
  ...
  -- Use := (scalar subquery), not SELECT...INTO STRICT — some parsers treat STRICT as a relation name here.
  trend_jsonb := (
    with months as (
...
comment on function public.superadmin_platform_stats is
  'Returns cross-tenant platform metrics only when JWT email is mysafeops@gmail.com.';
