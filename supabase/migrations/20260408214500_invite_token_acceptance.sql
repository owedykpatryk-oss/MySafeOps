-- MySafeOps: accept org invites by token (preferred), with email guard.
...
begin
  ...
  -- 1) Preferred: accept explicit invite token from URL flow.
  ...
  -- 2) Backward compatibility: accept first pending invite by email.
  select i.id, i.org_id, i.role
    into v_invite_id, v_org_id, v_invite_role
  from public.org_invites i
  ...
  -- 3) No invite: create own org with trial.
...
comment on function public.ensure_my_org(text, text) is
  'Returns org context; accepts invite token with email guard; otherwise creates org with 14-day trial.';
