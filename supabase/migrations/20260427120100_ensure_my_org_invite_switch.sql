-- Process invite tokens before returning an existing solo org (allows org switch on accept-invite).
...
comment on function public.ensure_my_org(text, text) is
  'Returns org context; invite token/email accepted before solo org return; else creates org with trial.';
